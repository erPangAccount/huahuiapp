<template>
	<view class="container">
		...
		<view class="mp-search-box">
			<input class="ser-input" type="text" placeholder="输入关键字搜索" confirm-type="search" @confirm="confirmSearch" />
		</view>
		<!-- #endif -->
		...
		<view class="cate-section">
			<view class="cate-item" v-for="item in categoryList" :key="item.id" @click="navToCategory(item.id)">
				<cmd-avatar :size="48" :text="item.firstCharOfName"></cmd-avatar>
				<text>{{item.name}}</text>
			</view>
		</view>
		<!-- 
		...
		<product-list :product="product.productList"></product-list>
	</view>
</template>
	...
	export default {
		...
		computed: {
			categoryList: function() {
				...
				for (let i in this.category) {
					needReturn[i] = {
						firstCharOfName: this.category[i].name.substr(0, 1),
		...
		methods: {
			...
			navToProductList(option = {}) {
				...
				if (queryStr.length) {
					uni.navigateTo({
						url: '/pages/product/list' + '?' + queryStr.replace(/(&)$/g, '')
					})
				} else {
					uni.navigateTo({
			...
			//获取商品分类
			getCategoryList: function() { //获取商品分类
				this.$load.loadding('加载中……');
				let token = this.$ajax.getToken();
				uni.request({
					method: 'GET',
					url: this.$ajax.ajaxBaseUrl + '/api/commodities_category',
					dataType: 'json',
					header: {
						'content-type': 'application/json',
						"Authorization": token.token_type + ' ' + token.access_token
		...
		// #ifndef MP
		// 标题栏input搜索框点击
		onNavigationBarSearchInputConfirmed: async function(val) {
			this.navToProductList({
				keyWords: val.text
			})
		},
		...
		onReachBottom: async function() {
			if (this.product.productCurrentPage < this.product.productLastPage) {
				this.getProductList({
					page: this.product.productCurrentPage + 1
				}, () => {
